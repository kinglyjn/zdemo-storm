package starter07.topology_fast_word_count;
...
public class FastWordCountTopology {
	...
	public static void main(String[] args) throws Exception {
		...
		config.registerMetricsConsumer(LoggingMetricsConsumer.class); // storm metrics

		String topoName = "FastWordCountTopology";
		if (args != null && args.length != 0) {
			StormSubmitter.submitTopologyWithProgressBar(args[0], config, builder.createTopology());
			
			//以下代码展示了如何在生产环境中kill拓扑
			Map<String, Object> stormConfig = Utils.readStormConfig(); // 读取本地storm-core包下的defaults.yaml及storm.yaml配置
			stormConfig.putAll(Utils.readCommandLineOpts()); // 读取命令行配置
			Nimbus.Client client = NimbusClient.getConfiguredClient(stormConfig).getClient();
			for (int i = 0; i < 10; i++) {
				Thread.sleep(60*1000); // Sleep for 1 mins
				printMetrics(client, topoName);
			}
			kill(client, topoName);
		} else {
			LocalCluster cluster = new LocalCluster();
			cluster.submitTopology(topoName, config, builder.createTopology());
			
			//以下代码展示了如何在本地环境中kill拓扑
